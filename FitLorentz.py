...
class FitLorentz(object):
    ...
    def fit_lorentzian_peak(wavelength, intensity, peak_index, window=5, maxfev=5000):
        ...
        try:
            popt_lorentzian, _ = curve_fit(FitLorentz.lorentzian, x, y, p0=[amp_init, cen_init, wid_init],
    ...
    def fit_contoh_lorentzian(wavelength, intensity, peak_index, window=5, maxfev=5000):
        if highest_peak_index != -1:
            x_zoom, y_zoom, fit_lorentzian_curve_zoom, _ = FitLorentz.fit_lorentzian_peak(wavelength, intensity, highest_peak_index)

            plt.figure(figsize=(10, 6))
            plt.plot(x_zoom, y_zoom, 'o', label=f'Data Puncak {wavelength[highest_peak_index]:.2f} nm')
            plt.plot(x_zoom, fit_lorentzian_curve_zoom, label=f'Lorentzian Fit {wavelength[highest_peak_index]:.2f} nm',
                     color='red')
...
for peak_index in peaks:
    x, y, fit_lorentzian_curve, popt_lorentzian = FitLorentz.fit_lorentzian_peak(wavelength, intensity, peak_index)

    if fit_lorentzian_curve is not None:
        plt.plot(x, y, 'o', label=f'Data Puncak {wavelength[peak_index]:.2f} nm')
        plt.plot(x, fit_lorentzian_curve, label=f'Lorentzian Fit {wavelength[peak_index]:.2f} nm')
...
FitLorentz.fit_contoh_lorentzian(wavelength, intensity, peak_index, window=5, maxfev=5000)